...
knitr::opts_knit$set(root.dir = "~/Documents/GitHub/resources/classes")
...
There er 20 data points with a mean of 1.5 and a standard deviation of 10. This should be normal distributed
...
plot_sim <- ggplot(my_simulated_df, aes(x = my_simulated_data)) + geom_histogram()
...
```{r}
summary(my_simulated_df)

sd(my_simulated_data)
```

We get different results from what we initially used. The smaller the sample, the less accurate it will be. We used normal distribution (rnom) and therefore it will never be one to one 
...
y_values_r <- rnorm(y_values,0,error)

df_ex1 <- data.frame(x_values,y_values_r)

ggplot(df_ex1, aes(x=x_values, y=y_values_r)) + geom_point() + geom_smooth(method = "lm", se = FALSE, color = "blue")  # Add a linear regression line
```
